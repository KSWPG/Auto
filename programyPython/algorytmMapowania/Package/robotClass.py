...
class robotClass:
	...
	def goTo(self,backPathMap):
		while (backPathMap[self.x][self.y] != 1):
			actualValue = backPathMap[self.x][self.y]

			if self.course == 0:
				if(self.x != 9 and backPathMap[self.x+1][self.y] == actualValue-1):
					self.go("R")
				elif(self.x != 0 and backPathMap[self.x-1][self.y] == actualValue-1):
					self.go("L")
				elif(self.y != 9 and backPathMap[self.x][self.y+1] == actualValue-1):
					self.go()
				elif(self.y != 0 and backPathMap[self.x][self.y-1] == actualValue-1):
					self.go("B")
